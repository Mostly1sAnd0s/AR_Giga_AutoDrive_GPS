 ...
 * 
 * NOTE: Delays added between reads to allow IMU to update registers.
 * Without delays, some sensors (especially magnetometer) return stale data.
 */
int IMU_I2C_ReadAll(imu_measurement_t *out)
{
    ...
    delayMicroseconds(500);  // Allow IMU to update registers
...
int IMU_I2C_ReadMagOffsets(float offsets[3]) {
    // We reuse the MAG read logic but interpret the values as offsets
    // after a calibration cycle. 
    return IMU_I2C_ReadMagnetometer(offsets);
}

int IMU_I2C_SetMagOffsets(float offsets[3]) {
    // If the sensor supports writing offsets via I2C, this would go here.
    // For many serial-to-I2C modules, we just keep them in local memory.
    // Returning 0 for now to satisfy the interface.
    return 0;
}
